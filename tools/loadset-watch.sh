...
#
# ══════════════════════════════════════════════════════════════════════════
# 🔴🔴 上条已被取代（委托人终裁「改」·2026-08-01 SGT·〔M〕490·JG-18 冲-1）
#   **原文一字不改地保留于上，照追加式原则不溯改。以下为取代之新分界。**
#
#   **新分界**：**裁载入集边界与执行令之权，自本裁起归 CGM**（原属 GM·-60③）。
#   本器自此**得决定迁什么**；`docs/adjudication-lanes.md` §四（规则四）为其正文。
#
#   🔴 **随权而来之两条硬约束（不因得权而松·反因得权而必守）**：
#   (1) **凡「删」与「移出」须列表呈报，不得静默生效**——逐项书移出何项／字节（须同书
#       读取时点）／改走何路径及其按需回读触发条件／新总量。周表落 `docs/loadset-slim/`。
#   (2) **宪法只得换载入版摘录件，正本一字不动**——**修宪权在 L0**，本裁未授、亦不得授
#       CGM 删改宪法正文之权；摘录件头须写死「凡作宪法层断言之前，必先回读正本」。
#
#   ⚠️ **残余风险如实注（CGM 自书·不粉饰）**：本裁使**执行侧得决定裁决侧看得见什么**。
#      此系结构性自利敞口，**不因主责转移而消失**；其唯一制衡即上列 (1) 之周表呈委托人
#      与 -60④「触发即 🔴，不得只记不办」。**若周表停发，本敞口即无制衡** —— 写死于此
#      以备后人审计。
# ══════════════════════════════════════════════════════════════════════════
#
...
#
#   🔴 **取代注（委托人终裁「改」·2026-08-01·〔M〕490·JG-18 冲-2·原文保留于上不溯改）**：
#     **定阈之权自本裁起归 CGM**（原「由其时之 GM 定」）。
#     **「须同书三代读数」一语不废、原样承继** —— 该条系数据纪律非权限归属，
#     **换了定阈之人不等于可以少一次读数**；三代读数未齐之前，CGM 亦不得定绝对上限。
#
# 🔴 委托人补本席之不足（-60④ 照录·不得删）：
...
print("\n## 🔴 本器不做之事\n")
# 🔴 CGM-G5 更正（2026-08-01 SGT）：本行原印「**不裁任何迁移边界**（-60③：GM 专权，CGM 不可代裁）」，
#   而本文件头之取代注（〔M〕490·JG-18 冲-1）已把该权改归 CGM——**取代注写进了文件头，而在跑的这一行还在印旧分界**。
#   此即本仓已实证之形态：「**条文改了而尺没改，下一轮报出来的还是旧数**」（〔M〕468②）。
#   **旧分界之原文一字不改地保留于本文件头**（追加式·不溯改）；此处系**运行时输出**，须与在效分界一致，否则本器每跑一次即散播一次已被取代的规则。
print("- ~~**不裁任何迁移边界**（-60③：GM 专权，CGM 不可代裁）~~ —— **已取代**（委托人终裁「改」·2026-08-01·〔M〕490·JG-18 冲-1）："
      "**裁边界与执行令之权已归 CGM**，本器**得决定迁什么**；**原文保留于本脚本文件头，不溯改**。"
      "**🔴 随权两约束不因得权而松**：(1) 凡删凡移出**须列表呈报**（周表落 `docs/loadset-slim/`）——**周表停发即视同权限中止**；"
      "(2) **宪法只得换载入版摘录件，正本一字不动**（修宪权在 L0，本裁未授）；")
print("- **不自行刷基线** —— `--set-baseline` 须有裁决来源；**基线一刷，增幅归零，红灯自然不亮**；")
print("- **不测绝对大小是否「过大」** —— 绝对上限未立；**定阈之权已归 CGM**（〔M〕490·冲-2），"
      "**惟 `-60②`「须同书三代读数」原样承继**——**换了定阈之人，不等于可以少一次读数**；三代读数未齐前不得定阈。**本器仍只测相对增幅。**")
PY
