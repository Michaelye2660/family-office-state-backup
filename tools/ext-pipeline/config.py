...
BANNED_PATTERNS = {
    ...
    # 裸数额变体（ADJ-0719-18②段二补强·币种可选式数额）：逗号千分位 或 量级后缀(万/亿/千/M/B/K)
    # ——排除前瞻挡掉计数/百分比/倍数(家/名/个/次/年/月/%/倍/个百分点等)·不追求语义完美(GM②·层2月度复扫+归档可审系二道网)
    "bare_amount": re.compile(
        r"(?:(?<![\d.])\d{1,3}(?:,\d{3})+(?![\d.])"                            # 逗号千分位 1,200,000
        r"|(?<![\d.\w])\d[\d,]*(?:\.\d+)?\s?(?:万|亿|千(?!瓦)|[MBKk](?![a-zA-Z])))"  # 量级后缀 250万/2.5M/3亿
        r"(?!\s?(?:%|％|倍|个百分点|个基点|家|名|个|人|位|次|届|种|类|项|条|页|字|年|月|日|周|瓦))"
    ),
}
