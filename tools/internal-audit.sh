...
PROM=$(cgm_add 'portfolio-state.md' 'adj-archive/*.md' 'gm-inbox/*.md' 'docs/*.md' 'docs/**/*.md' \
       | python3 -c "
import sys,re
for l in sys.stdin:
...
say "| 甲1-b · **⚠️ 本器之盲区（写死）** | — | **未用 \`〔承诺〕…〔落点：〕\` 标记之散文承诺，本器一律看不见** —— **散文里的两个字不能当判据**（同 R10 之限度）；**要它被量，就得把它写成可量的形状** |"
