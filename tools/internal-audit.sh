...
if [ -n "$CR" ]; then
  # 🔴 审查员复核所捕之 C 残口（其归因于自己上轮条文之字面，非 CGM）：
  #   原式作「含『已核』**而无**『未经维二』」→ **同一行内两者并存时反而 GREEN，
  #   而那恰是真正的混栏形态**（一行既说「已核」又说「未经维二」，读者取哪一个？）。
  #   **改为正向定义**：末栏须**恰等于**「知悉而不追认·未经维二」；**凡出现「已签／已核／维二通过」即红，
  #   不问同行有无「未经维二」**。
  MIX=$(grep -E '^\| *[0-9]+ *\|' "$CR" 2>/dev/null | grep -cE '已签|已核|维二通过' || true)
  NOMARK=$(grep -cE '^\| *[0-9]+ *\|' "$CR" 2>/dev/null || true)
  HASMARK=$(grep -E '^\| *[0-9]+ *\|' "$CR" 2>/dev/null | grep -c '知悉而不追认·未经维二' || true)
  ...
  # 🔴 审查员复核所捕：原作「无表→⚪ 不适用」＝**删档即免检**。
  #   **凡该档曾入库而今不在，即 🔴** —— 且**审计留痕之毁损自 2026-08-01 起落 G7，须呈 GM**。
  EVER=$(git log --oneline --diff-filter=A -- 'docs/internal-audit/*-carry-register.md' 2>/dev/null | wc -l | tr -d ' ')
  if [ "${EVER:-0}" -gt 0 ]; then
    RED=$((RED+1)); say "| 乙4-e · **结转登记之混栏核** | **档曾入库（${EVER} 次）而今不在** | 🔴 **删档即免检之形态** —— **审计留痕之毁损落 G7，须呈 GM** |"
  else
    say "| 乙4-e · **结转登记之混栏核** | **从未有过登记表** | ⚪ 不适用 |"
  fi
fi
...
# ── §三-补七 · 🔴 分流双项计数（`ADJ-0801-16④` 之实质迁移）──────────────
# 🔴 立此之由（**不是新增职能，是把一件悬置之义务搬到有授权的地方**）：
#   `routines/weekly-retro-sunday-prompt.md` 第1.10步 载 (a)〔乙〕项计数 ＋ (a-2) 清单外自行上呈计数；
#   **GM-13 于 `-16④` 裁：该 `routines/` 改动无 ADJ 亦无委托人一字，`routines/` 有明文特别规定在先，
#   特别不为一般（丙档「工具建改」）所吸收 —— 补 ADJ 或委托人一字之前，不得援为任何流程之依据。**
#   **委托人令「按 gm 决定执行」。**
#   **故本席不动 `routines/` 一字**（自 `-12` 起之承诺照旧），**改把两个计数落于本器** ——
#   **本器属丙档「工具建改」，无争议**；**其实质（两个数）系委托人 v1.0 §一 直令所立，不因载体悬置而落空**。
#   🔴 **须写明其限度**：**本节不使 `routines/` 那一行变为有效**；**它仍悬置，仍候 ADJ 或委托人一字。**
#   **本席只是不让「载体悬置」变成「义务消失」。**
say "## §三-补七 · **分流双项计数**（\`-16④\` 悬置期间之实质承载·**不使该 \`routines/\` 行变为有效**）"
say ""
say "| 项 | 本轮读数 | 判 |"
say "|---|---|---|"
YI=$(cgm_add 'adj-archive/*receipt*.md' 'docs/cgm-rulings-docket.md' | grep -cE '自判有疑|〔乙〕项' || true)
say "| (a) **〔乙〕项援引**（CGM 自判有疑而上呈） | ${YI} | ⚠️ **只出数不判** —— **长期为 0＝线过松（CGM 在硬扛判断）；长期偏高＝线过紧或 CGM 在推责**；**阈值未立，立阈值属判断类须委托人裁** |"
A2=$(cgm_add 'adj-archive/*receipt*.md' 'docs/cgm-rulings-docket.md' | grep -cE '清单外.{0,6}上呈|不该呈|本不该呈' || true)
say "| (a-2) **清单外自行上呈**（〔M〕494 立·N8/N9 为首例） | ${A2} | ⚠️ **只出数不判**·同上 |"
say ""
say "**🔴 本节之限度**：**它不使 \`routines/\` 第1.10步 变为有效** —— 该行仍悬置，仍候 ADJ 或委托人一字。"
say "**本席只是不让「载体悬置」变成「义务消失」。**"
say ""
...
# 🔴 射程扩容（审查员代核后·2026-08-01）：原式只扫 `say "…"`，**遂看不见 `python3 -c "…"` 内之反引号**——
#   而本日第三次同族缺陷恰发生在那里：`gen-submission-summary.sh` 之 python 块内反引号被 bash 执行，
#   **产出由 24,412 B 静默缩至 8,582 B，而该器照报「已生成」、退出码 0**。
#   **一把只量自己想得到的那一处的尺，其零命中不承载信息。** 故改扫**一切非注释行之双引号串**。
# 🔴 本项之射程曾于 2026-08-01 扩至「一切双引号串」，**当场退回**：
#   扩容版报 69 处，逐一查为**假红**——命中者皆在**单引号 heredoc／python 单引号串之内**（反引号在那里是安全的），
#   而**行级正则判不了引号上下文**。**一把永久报 69 的尺，几天内就没人看了**（审查员 §丙-3 之判据）。
#   **故收回为两条各自可靠的窄检查**，并如实标其合并射程仍有缺口。
# 🔴 射程由 `say` 扩至 `say|echo|printf`（2026-08-01·**本日反引号族第四次·而这次在 `echo` 里，1a 看不见它**）：
#   三者皆输出命令、同族、**射程仍窄仍可靠**——**不是又一次「扩到一切双引号串」那种不可靠的宽**。
UB=$(grep -nE '^[^#]*(say|echo|printf) "[^"]*[^\\]`' tools/*.sh 2>/dev/null | wc -l | tr -d ' ')
# 自检1-b：`python3 -c "` 之双引号块内之反引号 —— **本日第三次同族缺陷恰在此，而自检1 看不见它**
# 🔴 首版命中 4 处，逐一查为**假红**：全是**注释行与本判据自己那一行**——
#   **「判据档自身不入其所判之扫描面」之本日第六例，且这次是本条自己。**
#   **故只扫真正之调用行**：非注释 ∧ 该行确以 `python3 -c \"` 起一个双引号块。
UB2=$(grep -hE '^[^#]*python3 -c "' tools/*.sh 2>/dev/null | grep -v 'grep' | grep -c '`' || true)
UB=$((UB+UB2))
if [ "${UB:-0}" -gt 0 ]; then RED=$((RED+1)); R="🔴 **${UB} 处** —— 反引号在双引号内未转义即被 bash 当命令替换执行，**会把报告内容删空且不报错**"; else R="🟢 零处"; fi
say "| 自检1 · **未转义反引号**（1a \`say \"…\"\` ＋ 1b \`python3 -c \"…\"\`·**两条窄检查·合并射程仍不覆盖其它双引号串·如实标**） | ${UB:-0} | ${R} |"
