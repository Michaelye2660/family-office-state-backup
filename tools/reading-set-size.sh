...
# 🔴 载入集已由 ADJ-0731-58② 重定义（2026-07-31·委托人直令即时生效）——原**仅此六项**。
#    移出者（改按需回读）：memory-digests 全部逐日件与月档／裁决侧知识库全文／台账全文／cc-knowledge。
#    **本表系减法之实测载体**；照乙条（-50②）「凡增必读须报总量」，减亦须报。
# 🔴 ADJ-0801-04③ 补：**摘录件编为第 7 项**入载入集。
#    其非新增一整份必读项——正本 26,046 B 已于 -58② 移出，本项仅取其一部分回入（-04⑤ 走 (ii) 路之字节预算来源）。
MUST=[('宪法正本','docs/constitution.md'),
      ...
      ('下一代空骨架', _SK or '（尚未备）'),
      ('裁决侧知识库·摘录件〔**第 7 项·-04③ 立**〕','docs/adjudicator-knowledge-DIGEST.md')]
DIGEST_P, DIGEST_LIMIT = 'docs/adjudicator-knowledge-DIGEST.md', 4500
...
print(f"| **合计（载入七项·八件）** | **{tot:,}** | **不含已移出者·见下** |\n")

# 🔴 第 7 项之上限核（-04③ 定 4,500 B·「超限即触发再裁，不得默默放宽」）
_db = os.path.getsize(DIGEST_P) if os.path.exists(DIGEST_P) else 0
if not _db:
    print(f"**🔴 第 7 项 `{DIGEST_P}` 不在库** —— 载入集缺件。\n")
elif _db > DIGEST_LIMIT:
    print(f"**🔴 第 7 项超限**：{_db:,} B ／ 上限 {DIGEST_LIMIT:,} B（**超 {_db-DIGEST_LIMIT:,} B**）。"
          f"照 `-04③`「**超限即触发再裁，不得默默放宽**」，**本器只报不自削**；已呈裁，未获裁前该件按现状计入合计。\n")
else:
    print(f"**🟢 第 7 项未超限**：{_db:,} B ／ 上限 {DIGEST_LIMIT:,} B（余量 {DIGEST_LIMIT-_db:,} B）。\n")
...
print()
print(f"**🔴 本数系 `-04⑤` 所令之实测**（GM 侧无仓库 shell，明书「**本席不以估算充实测**」）：加入第 7 项后总量 **{tot:,} B**；"
      f"其中第 7 项占 {_db:,} B（{(_db/tot*100 if tot else 0):.1f}%）。")
print()
...
print("　　🔴 **取代注（委托人终裁「改」·2026-08-01·〔M〕490·JG-18 冲-2·上句原文保留不溯改）**："
      "**定阈之权自本裁起归 CGM**（原 `ADJ-0731-60②`「由其时之 GM 定」）；"
      "**惟「须同书三代读数」原样承继**——该条系数据纪律非权限归属，**换了定阈之人不等于可以少一次读数**，"
      "三代读数未齐之前 CGM 亦不得定绝对上限。正文见 `docs/adjudication-lanes.md` §四。")
