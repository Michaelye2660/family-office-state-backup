...
MUST=[('宪法正本','docs/constitution.md'),
      ...
      ('下一代空骨架', _SK or '（尚未备）')]
# 🔴 呈裁项：gm-snapshot 于 -58② 两份清单内皆未出现（既未列入载入六项，亦未列入不载入四项）。
#    其系 ADJ-0721-12 之「步0 每次顺读」项。**本器不擅定其归属，改为两个总量并报。**
SNAP=('gm-snapshot〔**归属未定·呈裁**〕','docs/gm-snapshot.md')
...
print(f"| {SNAP[0]} | {snap_b:,} | **不计入上方合计**——归属未定，见下 |")
print()
print(f"**两个总量并报（因 gm-snapshot 归属未定·本器不擅定）**：**不含 {tot:,} B** ／ **含 {tot+snap_b:,} B**。")
print()
