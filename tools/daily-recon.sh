...
for l in rows:
    ...
    if not ds:
        print(f"NODATE|{obj}")   # 非日期型触发条件（如「第2个财报季结束后30日内」）→ 本脚本不判，留周频人核
        continue
    t=ds[0]
    if t <= d: print(f"{t}|{obj}")
    else:      print(f"FUTURE|{obj}")
PY
)
N3=$(printf '%s' "$OV" | grep -vc '^\(NODATE\|FUTURE\)|' || true)
# 🔴 CGM-G5 更正（2026-08-01 SGT）：③b 原以 `grep -c 待执行 全档 - 逾期数` 计，
#   **其标签书「非日期型」而其算法实为「非逾期」** —— 未来日期之日期型行（如再评点 2026-08-19／08-31）
#   会被计进「非日期型·须人核」，而它们恰恰是**最不须人核**的一类（有确定触发日，源③ 到点自会报）。
#   **一个把「已妥善挂钟的项」算成「须人工盯的项」的计数，会让人核栏长期虚高而无人当真。**
#   形态同族：本日第四次「标签与尺不符」。现改为**逐行判型**，两类分列，并各自申报其边界。
NND=$(printf '%s' "$OV" | grep -c '^NODATE|' || true)
NFU=$(printf '%s' "$OV" | grep -c '^FUTURE|' || true)
if [ "$N3" -gt 0 ]; then R="🔴 逾期 $(printf '%s' "$OV" | grep -v '^\(NODATE\|FUTURE\)|' | tr '\n' ';')"; RED=$((RED+1)); else R="🟢 无逾期"; fi
say "| ③ | 〔K-0〕触发日≤${D} 而仍「待执行」 | ${N3} | ${R} | 逐项按表 |"
say "| ③b | 〔K-0〕**非日期型**触发条件之「待执行」行（如「第2个财报季结束后30日内」「Q3电话会时」）——**本脚本判不了、留周频人核**·不得因未标红而当作无事 | ${NND} | ⚠️ 须人核 | GM |"
say "| ③b-2 | 〔K-0〕**日期型·触发日在 ${D} 之后**（已妥善挂钟·**到点由源③ 自报，不须人核**） | ${NFU} | 🟢 在钟 | 逐项按表 |"
...
say "| ③d | **⚠️ 本闸只测日期型挂钟行** —— 非日期型触发条件（见 ③b ${NND} 行）**本器判不了**；**不得因 ③c 绿而认为闸已全测** | — | ⚠️ 半测 | GM |"
