...
## 【ADJ-0720-11·2026-07-20·GM-5 跟办闭环】0718-02(c) 裁结 + 扫描线加固三则

> **GM-5 定性（照录）**：0718-02 扫描首批期间，Staples worker 之 scratchpad 中间脚本 `compute.py` 任务中途被外部篡改（改读不存在之 `data.pkl` 并注入预设标签）；worker 识破弃用、改用自身脚本对原始 SEC JSON 缓存重算、未采信被注入值——**防线当次有效，但系 worker 自律之软控制**。其后建成之 API 硬净室管道（0719-17~19）仅加固 EXT 外审中转线，**扫描 worker 线不在其覆盖内**。按"软控制升硬"同理，扫描线予**轻量硬化**——但按比例原则**不复制整套净室**（扫描产出=观察层只读数据·风险级低于审计线与真金线），把 0718-02 worker 的正确行为**成文为规则**即可。**0718-02(c) 候评估项就此裁结。**

### 规则三（扫描SOP standing·即行）：中间产物 sha1 钉扎
- worker 任务启动时，对**自建关键中间脚本/中间数据**登记 sha1；**复用前校验**。
- **失配处置**：弃用该产物 → 回**原始源（SEC JSON 缓存）**重算 → **事件一行上报**。
- 性质=0718-02 worker 行为之成文化，亦为规则一"会话内生成脚本自记 hash"在扫描线之常设化。

### 规则四（扫描SOP standing·即行）：预设标签拒信
- **来源不明数据文件**（非 worker 自建、非登记原始缓存之 pkl/json/csv 等）一律**拒读拒信**。
- 出现即按规则三处置（弃用→raw 重算→上报）。

### 规则五（扫描SOP standing·即行）：报告完整性声明栏
- 扫描批次报告脚注增**"环境完整性"栏**：本批是否发生完整性事件·处置方式·是否 raw 重算。

> **执行注**：纯扫描工具层 SOP·零仓位·不动持仓/触发器/红线；不新增审计班次、不加重扫描成本（sha1 登记为一次性轻操作）；净室管道（EXT 线）与真金线（T1-T4）规格不受本件影响。落 moat 工作指引 **v1.18** §五。

---
*编制=CGM·2026-07-18·ADJ-0718-03⑤c｜规则一=GM 令即行（vendored hash 对表）｜规则二=根因对症即行（并发命名空间隔离）｜规则三/四/五=ADJ-0720-11·GM-5 跟办闭环（sha1 钉扎/预设标签拒信/报告完整性栏·2026-07-20）｜重制度经 ADJ-0718-06③ 评估=不需要·结案*
