...
- **探针三·二手源伪装一手源**:夹带**一项我方已知其「一手外观」实为转发者**之数据。**正确回答=标为二手源,或明说「无法确认底层来源」**;**若标为「一手源」→ 整份答卷可信度下修**(不作废,但其余各项之「一手」标注一律不予采信,须我方逐项另核)。
  **⚠️设计纪律(二条·第②条系 DR-01 v3 实战暴露后所加·2026-07-28·〔M〕341④(d))**:
  ①**实例必须是我方已实际核实者**,**不得凭记忆点名某站为「已知转发者」**——否则探针本身即成脑补(ADJ-0727-02⑧／安全约束⑤);
  ②**🔴实例必须是受测方在其可及信息内能够判定者**——若该判定依赖**我方独有之调用条件**(如已订阅之付费接口输出),则该题在受测方信息集下**无正确答案**,**测的就不再是判定能力,而是取数权限**;**判读时须先问「它有可能答对吗?」——答否即该枚本轮不计分并须换实例**。
  **实证**:DR-01 v3 之探针三——我方钥匙(Bigdata 底层=FMP)系**调用该工具直接观测所得**(输出印有 `*Source: [FMP](...)*`),而受测方未订阅该接口、仅能查其公开文档,**文档确未点名底层提供商**→其答「无法确认」**在其信息集下即最优解**;**该枚之主判定(二手/聚合)仍过,惟归因子题作废**。
...
**在途件二 · `DR-20260728-02`**(relay `2443091`·2026-07-28 投放·候答卷):短久期美债 **UCITS ETF 候选对照表**(八栏=发行人／ISIN／上市代码／TER／AUM／复制方式／证券借贷政策／**存管人法人全称**)·**🔴硬性排除 iShares–BlackRock 家族**;起因=委托人「换」终裁(ADJ-0728-13①)之工具遴选。
**🔴路径纪律入表(本轮实测所得)**:Bigdata `find_securities` 以「short term US treasury UCITS ETF」及具体代码检索,**两次均返回空结果集**——**该库对爱尔兰 UCITS 覆盖差**(同症已现于 DR-01:IWDA 须以 `SWDA` 方能命中);**故此后凡 UCITS 工具遴选类取数,默认直走 DR 轨,不再先试该库**。
...
## 🔴 八、FMP 已知边界（**并列登记·凡取数前先读本节，免重复试错**）

| # | 边界 | 实证 | 处置 |
|---|---|---|---|
| 1 | **Starter 档只覆盖美国上市标的** | `0700.HK`／`0005.HK`／`IWDA.L`／`SGLN.L`／`IGLN.L`／`159516.SZ`／`159819.SZ`／`159142.SZ` **一律在参数层被拒**（2026-07-30 实测） | 直走第 2 层网页（腾讯自选股／iShares 官网／中证官网） |
| 2 | `batch-quote` 需 Premium+ | 2026-07-30 实测 | 单标的 `quote` 可用 |
| 3 | `earningsTranscript` 需 Ultimate | 2026-07-31 实测 | 走 SEC EDGAR 之 8-K Ex-99.1 |
| 4 | **🔴 `analyst/price-target-consensus` 含陈旧未更新条目**（ADJ-0801-08④ 令登记·2026-08-01） | 该端点报 SPGI `targetHigh` ＝ **625**，与 **Evercore 于 2026-04 已下调至 560 之前值**吻合 → **含未更新条目**；其共识 **541.22** 显著高于第 2 层五源之 **518–525** 一档。**财报后各家实际动作清一色落在 500–553**（JPM 555→530／Baird 521→513／Barclays 555→553／Clear Street 维持 516／Citi 454→500） | **🔴 凡以该端点作安全垫分子者，须与第 2 层至少一源交叉；单用该端点得出之安全垫不得作门槛判定。** |

**🔴 第 4 项之形态（值得与第 1–3 项分开记）**：第 1–3 项系**取不到**（参数层被拒，**失败是响的**）；
**第 4 项系取得到但值是旧的** —— **它不报错，它给你一个数**。
**一个会安静给出陈旧值的端点，比一个会拒绝你的端点危险**：前者只在你交叉核对时才暴露，而安全垫之计算恰恰是最不容易被交叉核的一步（分子只有一个数，看起来无从对照）。

**并及 `-08⑤` 之一手归一实测（同轮·SPGI Q2'26·**一手＝8-K Ex-99.1·SEC EDGAR accession `0000064040-26-000040`**）**：
...
**三源三数之归一结论**：FinancialContent 4.83 ✅／TradingView 4.83 ✅／**Google Finance 4.87 ❌ 无一手对应**。
**且三源所报皆系「调整后」而非 GAAP** —— **GAAP 与调整后差 $0.71（17%）**；**凡引 SPGI EPS 未标口径者，不可用。**

**🔴 一手所不能归一者，如实报**：**预期值之分歧（5.02 vs 4.81）一手归不了** —— 公司新闻稿只载实际值，**不载市场共识**。
故 **EPS surprise 之分母仍系多源且未归一**；**在其归一前，「EPS surprise −3.7%」不得作门槛判定，只可作方向参考并须同书其分母来源。**
